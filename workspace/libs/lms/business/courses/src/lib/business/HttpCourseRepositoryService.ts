...
export class HttpCourseRepositoryService extends ServiceBase {
  ...
  retrieveLatestCourses<T>(): Observable<ApiResponse<T>> {
    this.loggingService.log(
      ...
      `Preparing to execute the API call for the latest video courses.`
    );
    const courses: Course[] = [];
    const course = new Course();
    course.author = new Author();
    course.author.bio =
      'Loves tacos, jazz, and Angular. Building enterprise applications using CLEAN principles and patterns.';
